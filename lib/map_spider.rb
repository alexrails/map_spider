...
class MapSpider
  ...
  def initialize
    @client = GooglePlacesClient.new(ENV['GOOGLE_MAPS_API_KEY'])
    @all_places = []
  end
  ...
  def start
    ...
    coordinates.each_with_index do |coordinates, index|
      @scanned_area = 0
      catch_places(coordinates, radius, radius)
    end
  ...
  def catch_places(coordinates, radius, square_size)
    return if @stop_scan
    ...
    logger.info("Scanned point: #{coordinates[:lat]}, #{coordinates[:lng]}, Radius: #{radius}, Places found: #{places.count}, Requests: #{requests_counter}")
